...
contract DEXPair is ERC20 {
    ...
    // Variables para metadatos dinámicos
    string private customName;
    string private customSymbol;

    // Sobrescribir name y symbol para que sean dinámicos
    function name() public view override returns (string memory) {
        return bytes(customName).length > 0 ? customName : super.name();
    }

    function symbol() public view override returns (string memory) {
        return bytes(customSymbol).length > 0 ? customSymbol : super.symbol();
    }
    ...
    function initialize(address _token0, address _token1) external {
        ...
        // Generar metadatos dinámicos basados en los tokens
        try ERC20(_token0).symbol() returns (string memory s0) {
            try ERC20(_token1).symbol() returns (string memory s1) {
                customName = string(abi.encodePacked(s0, "-", s1));
                customSymbol = string(abi.encodePacked(s0, "-", s1));
            } catch {
                customName = "DEX LP Token";
                customSymbol = "DEX-LP";
            }
        } catch {
            customName = "DEX LP Token";
    ...
    function swap(uint amount0Out, uint amount1Out, address to) external {
        ...
        require(
            balance0Adjusted * balance1Adjusted >=
                uint(_reserve0) * uint(_reserve1) * 1000 * 1000,
            "K"
        );
